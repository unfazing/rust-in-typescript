...
// TODO: type checker currently unable to tell if moving a type that is stored in an array (add a field to Type?)
test_typechecker(`
    fn main() {
        let arr: [&string; 3] = [&"1", &"2", &"3"];
        let test: string = *arr[2];
    }
    `, "cannot move out of type [string ; size 3], a non-copy array", "Invalid - Moving from a non copy array")
...
test_typechecker(`
    ...
    `, "Type error in assignment; cannot move out of type [string ; size 3], a non-copy array", "Invalid - Move non copy string out of an array")
